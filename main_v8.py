            ...
            if not self.game_over:
                ...
                for obstacle in self.obstacles:
                    if llama_rect.colliderect(obstacle.get_rect()):
                        self.game_over = True
                        seconds_survived = round(self.ticks_survived / 60, 1)
                        if seconds_survived > self.highscore:
                            self.update_high_score(seconds_survived)
                            self.highscore = seconds_survived
                        break
                self.ticks_survived += 1
                seconds_survived = self.ticks_survived / 60
                score_text = self.small_font.render(f"Time: {seconds_survived:.1f}s", True, white)
                self.screen.blit(score_text, (20, 10))
                highscore_text = self.small_font.render(f"High Score: {self.highscore:.1f}s", True, white)
