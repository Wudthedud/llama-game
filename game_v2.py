    ...
    def update_game(self, bg):
        ...
        if self.difficulty_timer % 90 == 0:
            ...
            if self.obstacle_speed < self.max_obstacle_speed:
                self.obstacle_speed = min(self.obstacle_speed + 0.4, self.max_obstacle_speed)
            for o in self.obstacles:
        ...
        if self.spawn_timer >= self.spawn_interval:
            ...
            if group_chance < 0.25:
                ...
                for i in range(group_size):
                    ...
                    self.obstacles.append(Obstacle(x, y, scale, self.obstacle_speed))
            else:
                scale = random.choice([1, 0.9, 0.8, 0.7, 0.6])
                y = random.choice([0, 10, 20, 30, 40, 50, 60])
                self.obstacles.append(Obstacle(1000, y, scale, self.obstacle_speed))
            variation = random.randint(-20, 20)
            self.spawn_interval = max(self.min_spawn_interval, self.spawn_interval + variation)
